...
//alert("지금 현재 페이지를 찍어봅시다. " + currentPage);
		...
		callAjax("/community/FreeBoardList.do","post","text", true, param, resultCallback);
	...
	function fOnloadImg(param){
	 ...
	 function fNoticeModal(freeId) {
		 ...
		 if(freeId == null || freeId==""){
			...
			$("#action").val("I"); // insert 
			frealPopModal(freeId); // 공지사항 초기화 
			
			//모달 팝업 모양 오픈! (빈거) _ 있는 함수 쓰는거임. 
			gfModalPop("#freeboard");
			
		 }else{
			// Tranjection type 설정
			$("#action").val("U");  // update
			fdetailModal(freeId); //번호로 -> 공지사항 상세 조회 팝업 띄우기
		 }
	 ...
	 /*공지사항 상세 조회*/
	 function fFreeModal(freeId){
		 //alert("공지사항 상세 조회  ");

		 var param = {freeId : freeId};
		 var resultCallback2 = function(data){
			 fdetailResult(data);
		 };
		 
		 callAjax("/community/detailBoard.do", "post", "json", true, param, resultCallback2);
		 //alert("공지사항 상세 조회  22");
	 }
	 ...
	 function fdetailResult(data){
		 ...
		 if(data.result == "SUCCESS"){	 
			...
			// 모달에 정보 넣기 
			frealPopModal(data.detailBoard);
	 ...
	 function frealPopModal(object){
		 ...
		 if(object == "" || object == null || object == undefined){
			 ...
			 $("#freeTitle").val(object.freeTitle);
			 $("#freeContent").val(object.freeContent);
			//////object.noticeNo
			 
			 $("#freeId").val(object.freeId); // 중요한 num 값도 숨겨서 받아온다. 
			 
			 var swriter = $("#swriter").val();
			 
			// alert(object.loginID + " : " + swriter );
			 
			 if(object.loginID == swriter) {
	 ...
	 function fSaveNotice(){
		 ...
		 callAjax("/community/boradSave.do", "post", "json", true, $("#myNotice").serialize(), resultCallback3);
	 ...
	 function fDeleteNotice(){
		 ...
		 if(con){
			 ...
			 $("#action").val("D");  // delete
			 callAjax("/community/boardDelete.do", "post", "json", true, $("#myNotice").serialize(), resultCallback3);
...
<form id="myNotice" action="" method="">
	...
	<input type="hidden" id="swriter" name="swriter" value="${curloginID}"> <!-- 작성자 session에서 java에서 넘어온값 -->
				...
				<li class="contents">
					...
					<div class="content">
						...
						<p class="Location">
							...
							<a href="#" class="btn_nav bold">커뮤니티</a> 
	...
	<!-- 모달팝업 -->
	<div id="freeboard" class="layerPop layerType2" style="width: 600px;">
		<input type="hidden" id="freeId" name="freeId" value="${freeId}"> <!-- 수정시 필요한 num 값을 넘김  -->
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3"><input type="text" class="inputTxt p100"
								name="freeTitle" id="freeTitle" /></td>
						</tr>
						<tr>
							<th scope="row">내용</th>
							<td colspan="3">
								<textarea class="inputTxt p100" name="freeContent" id="freeContent">
								</textarea>
							</td>
						</tr>
