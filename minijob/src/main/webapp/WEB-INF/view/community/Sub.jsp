...
//alert("지금 현재 페이지를 찍어봅시다. " + currentPage);
		...
		callAjax("/community/subList.do","post","text", true, param, resultCallback);
	...
	function fOnloadImg(param){
	 ...
	 //강의등록
	   function fSaveNotice(data) {
	        ...
	        if(confirm("등록하시겠습니까?")){
	            ...
	            callAjaxFileUploadSetFormData("/community/subSave.do", "post", "json", true, dataWithFile, resultCallback);  
	         }else {
	               return;
	        }
	   }
	 ...
	 function fNoticeModal(supId) {
		 ...
		 if(supId == null || supId==""){
			...
			// Tranjection type 설정
			$("#action").val("U");  // update
			fdetailModal(supId); //번호로 -> 공지사항 상세 조회 팝업 띄우기
		 }
	 ...
	 /*공지사항 상세 조회*/
	 function fdetailModal(supId){
		 //alert("공지사항 상세 조회  ");
		 var param = {supId : supId};
		 var resultCallback2 = function(data){
			 fdetailResult(data);
		 };
		 
		 callAjax("/community/detailSub.do", "post", "json", true, param, resultCallback2);
		 //alert("공지사항 상세 조회  22");
	 }
	 ...
	 function frealPopModal(object){
		 ...
		 if(object == "" || object == null || object == undefined){
			 ...
			 $("#supTitle").val(object.supTitle);
			 $("#supContent").val(object.supContent);
			//////object.noticeNo
			 
			 $("#supId").val(object.supId); // 중요한 num 값도 숨겨서 받아온다. 
	 ...
	 function fUpdateNotice(){
		 ...
		 callAjax("/community/subSave.do", "post", "json", true, $("#myNotice").serialize(), resultCallback3);
	 ...
	 function fDeleteNotice(){
		 ...
		 if(con){
			 ...
			 $("#action").val("D");  // delete
			 callAjax("/community/subDelete.do", "post", "json", true, $("#myNotice").serialize(), resultCallback3);
...
<form id="myNotice" action="" method="">
	...
	<div id="notice" class="layerPop layerType2" style="width: 600px;">
		<input type="hidden" id="supId" name="supId" value="${supId}"> <!-- 수정시 필요한 num 값을 넘김  -->
		...
		<dl>
			...
			<dd class="content">
				...
				<table class="row">
					...
					<tbody>
						...
						<tr>
							...
							<td colspan="3"><input type="text" class="inputTxt p100"
								name="supTitle" id="supTitle" /></td>
						</tr>
						<tr>
							<th scope="row">내용</th>
							<td colspan="3">
								<textarea class="inputTxt p100" name="supContent" id="supContent">
								</textarea>
							</td>
						</tr>
						<tr>
					...
					<td colspan="3">
					<input type="file" class="inputTxt p100" id="file" name="file"">
					</td>
						</tr>
					</tbody>
				</table>
