...
<script type="text/javascript">
	...
	function fcnsList(currentPage) {
		...
		console.log("stddate : " + $("#stddate").val());
		...
		callAjax("/conSupport/projectList.do", "post", "text", true, param, resultCallback);
	}
	//프로젝트 리스트 과정콜백
	function fcnsListResult(data, currentPage) {
		...
		var totalCntConSupportB = $("#totcnt").val();

		
		//페이지설정
		var paginationHtml = getPaginationHtml(currentPage, totalCntConSupportB,
	...
	//상세페이지 이름 검색
	function fListperson(currentPage, pro_no) {
		...
		var resultCallback = function(data) {
			...
			fpersonListResult(data, currentPage);
		};

		callAjax("/conSupport/nameList.do", "post", "text", true, param, resultCallback);
	} 
	
	//상세페이지 이름 검색 과정콜백
	function fpersonListResult(data, currentPage) {
		console.log("data : " + data);
		

		$('#cnsUserDtlList').empty();
		$('#cnsUserDtlList').append(data);
		
		var totalCntConSupportB = $("#totalCntDtlList").val();

		var paginationHtml = getPaginationHtml(currentPage, totalCntConSupportB,
				pageSizeDtlList, pageBlockSizeDtlList, 'fListperson');
	...
	function fSelectDtlCns(loginID, lec_seq) {
		...
		callAjax("/consupport/selectNameList.do", "post", "json", true, param,
				resultCallback);
	}
	//단건 유저 뽑기 콜백
						...
						<div class="content">
							<p class="Location">
								<a href="#" class="btn_set home">메인으로</a> <a href="#"
									...
									기업 지원현황 </span> <a href="#" class="btn_set refresh">새로고침</a>
							</p>
							
							<p class="conTitle">
								<span id="">기업 지원현황</span>
							</p>
							...
							<p class="search">
								<select id="searchKey" name="searchKey" style="width: 100px;">
									<option value="lec_nm" id="option1" selected="selected">프로젝트명</option>
