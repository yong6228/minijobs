...
function fModelNameSearch(){
	...
	let url = "/ged/salesStatus.do/list/total";
...
function fSelectTR(user_company, currentPage){

	$("#selectedCompanyName").val(user_company);
	...
	// 사용자가 데이트 피커를 건드린적이 있는가
	// 0이면 안건드림, 1이면 건드림
	let datePickerFlag = $("#datePickerClieckedFlag").val();
	...
	if(datePickerFlag == 0){
	  ...
	  var resultCallback = function(data) {
		  fOrderDetailList(data, currentPage);
	  };
	  // Ajax실행 방식
	  // callAjax("Url",type,return,async or sync방식,넘겨준거,값,Callback함수 이름)
	  // html로 받을거라 text
	  callAjax("/ged/salesStatus.do/list/detail", "post", "text", true, param, resultCallback);
}

	/** 일별 수주 내역 조회 콜백 함수 */
function fOrderDetailList(data, currentPage) {
	  ...
	  let seletedCompanyName = $("#selectedCompanyName").val();
	  ...
	  // 현재 페이지 설정
	  $("#currentPageCompanyDetail").val(currentPage);
}
...
function fListDailyOrderHistroy(currentPage) {
  ...
  // callAjax("Url",type,return,async or sync방식,넘겨준거,값,Callback함수 이름)
  // html로 받을거라 text
  callAjax("/ged/salesStatus.do/list/total", "post", "text", true, param, resultCallback);
}
